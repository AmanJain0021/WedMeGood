    ...
    <div id="global-topbar" className="bg-white/80 backdrop-blur-md fixed top-0 left-0 right-0 z-50 h-16 px-2 sm:px-4 lg:px-6 border-b border-slate-100 flex items-center justify-between gap-2 sm:gap-4 transition-all duration-300">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 cursor-pointer group min-w-0" onClick={() => navigate('/vendor/dashboard')}>
          ...
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-1">
              <span className="text-[12px] xs:text-[13px] sm:text-[15px] font-serif font-black text-slate-900 leading-tight truncate max-w-[140px] xs:max-w-[180px] sm:max-w-[220px] lg:max-w-none">{businessName}</span>
              <Icon name="verified" size="xs" className="text-violet-500 flex-shrink-0" />
            </div>
            <span className="text-[8px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none mt-0.5 truncate">Vendor Portal</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0 mr-3 sm:mr-5">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            {upcomingBookings.length > 0 && (
              <span className="absolute top-0 right-0 h-3.5 w-3.5 rounded-full bg-violet-600 text-white text-[8px] font-black flex items-center justify-center border-2 border-white shadow-sm animate-pulse">
          ...
          {showBookingsDropdown && (
            <div className="absolute right-0 top-11 mt-2 w-[290px] sm:w-[320px] bg-white border border-slate-100 shadow-xl rounded-2xl p-3 z-[100] animate-in fade-in slide-in-from-top-2 duration-200 select-none">
              <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-50">
                <span className="text-[11px] font-black text-slate-800 uppercase tracking-wider">Upcoming Bookings</span>
                <span className="text-[9px] font-bold text-violet-500 bg-violet-50 px-2 py-0.5 rounded-full border border-violet-100/50">
              ...
              {upcomingBookings.length > 0 ? (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="w-9 h-11 bg-slate-50 rounded-lg flex flex-col items-center justify-center border border-slate-100 flex-shrink-0">
                          <span className="text-[6.5px] font-extrabold text-violet-500 leading-none uppercase">{monthStr}</span>
                          <span className="text-[12px] font-black text-slate-700 leading-none my-0.5">{dateVal}</span>
                          <span className="text-[6.5px] font-extrabold text-slate-400 leading-none uppercase">{dayStr}</span>
                        </div>

                        {/* Customer Details */}
                        <div className="flex-1 min-w-0">
                          <p className="text-[11px] font-black text-slate-800 truncate leading-tight tracking-tight">{book.customerName}</p>
                          <p className="text-[8.5px] font-bold text-slate-400 truncate leading-none mt-0.5">{book.services?.join(', ') || 'Wedding Event'}</p>
                          <div className="flex items-center gap-0.5 text-[8px] font-semibold text-slate-400 mt-1 truncate">
                        ...
                        <div className="flex-shrink-0 text-right">
                          <span className="text-[10px] font-extrabold text-violet-600 bg-violet-50 px-1.5 py-0.5 rounded border border-violet-100/30">
                ...
                <div className="py-8 flex flex-col items-center justify-center text-center space-y-2">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-slate-700 uppercase tracking-wider">No Bookings Yet</p>
              ...
              <div className="mt-2.5 pt-2 border-t border-slate-50">
                <button 
                  className="w-full h-8 text-[9px] font-black text-white bg-violet-600 rounded-lg hover:bg-violet-700 active:scale-98 transition-all flex items-center justify-center gap-1 uppercase tracking-widest shadow-xs"
                  onClick={() => {
                    setShowBookingsDropdown(false);
                    navigate('/vendor/bookings');
                  }}
        ...
        >
          ...
          <span className="absolute top-0 right-0 h-3.5 w-3.5 rounded-full bg-rose-500 text-white text-[8px] font-black flex items-center justify-center border-2 border-white shadow-sm">3</span>
        </button>
      </div>
    </div>
  );
